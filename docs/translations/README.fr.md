...
Un système de tickets de support complet et intégrable pour AdonisJS v6. Intégrez-le dans n'importe quelle application — obtenez un helpdesk complet avec suivi des SLA, règles d'escalade, workflows des agents et un portail client. Aucun service externe requis.
...
- **Ticket Snooze:** Mettre en veille les tickets avec des préréglages (1h, 4h, demain, semaine prochaine) ; la commande Ace `node ace escalated:wake_snoozed_tickets` les réactive automatiquement
- **Saved Views / Custom Queues:** Save, name, and share filter presets as reusable ticket views
- **Embeddable Support Widget:** Widget léger `<script>` avec recherche dans la base de connaissances, formulaire de ticket et vérification de statut
...
- **Real-time Broadcasting:** Diffusion optionnelle via AdonisJS Transmit avec repli automatique sur le polling
...
- **[Escalated for AdonisJS](https://github.com/escalated-dev/escalated-adonis)** — Package AdonisJS v6 (vous êtes ici)
