...
var FactoryHardTimeout = class _FactoryHardTimeout extends Exception {
  static code = "E_FACTORY_HARD_TIMEOUT";
  static message = "Factory has timed out after waiting for hard timeout";
  key;
  constructor(key) {
    super(_FactoryHardTimeout.message, { code: _FactoryHardTimeout.code });
    this.key = key;
  }
};
...
var cacheOperation = diagnostics_channel.tracingChannel("bentocache.cache.operation");
...
var FactoryRunner = class {
  #locks;
  #stack;
  #skipSymbol = /* @__PURE__ */ Symbol("bentocache.skip");
...
var TwoTierHandler = class {
   ...
   */
  #returnL1Value(key, item, message) {
    if (message) {
      message.hit = true;
      message.tier = "l1";
   ...
   */
  async #returnRemoteCacheValue(key, item, options, message) {
    if (message) {
      message.hit = true;
      message.tier = "l2";
    }
  ...
  #returnGracedValueOrThrow(key, item, options, err, message) {
    if (options.isGraceEnabled() && item) return this.#returnL1Value(key, item, message);
    throw err;
  }
  async #applyFallbackAndReturnGracedValue(key, item, layer, options, message) {
  ...
  async #lockAndHandle(key, factory, options, localItem, message) {
    let releaser;
    try {
  ...
  handle(key, factory, options) {
    ...
    return cacheOperation.tracePromise(async () => {
      if (options.forceFresh) {
        return this.#lockAndHandle(key, factory, options, void 0, message);
      }
...
var SingleTierHandler = class {
  ...
  async #applyFallbackAndReturnGracedValue(key, item, options, message) {
    if (options.grace && options.graceBackoff) {
      this.logger.trace(
        { key, cache: this.stack.name, opId: options.id },
        "apply fallback duration"
  ...
  async #handleInternal(key, factory, options, message) {
    let remoteItem;
...
var Bus = class {
  ...
  constructor(name, driver, logger, emitter, options = {}, internalOperationWrapper) {
...
var RemoteCache = class {
  ...
  #wrapInternalOperation(fn) {
    return this.#options.internalOperationWrapper ? this.#options.internalOperationWrapper(fn) : fn();
  }
  /**
